      ...
      <a-row>
        <a-col :span="24">
          <a-card :loading="loading" activeTabKey="id" :bordered="false" title="训练成绩报表" :style="{ marginLeft:'10px',marginTop: '24px' }">
            ...
            <line-chart-multid :fields="trainingFields" :dataSource="trainingInfo100"></line-chart-multid>
          </a-card>
        </a-col>
      </a-row>
    </div>
...
<script>
    ...
    data() {
      return {
        columns: [],
        url: {
          list: "/statistics/statistics/listD100aByMainId",
          chart:"/statistics/statistics/trainingInfo100",
          delete: "/statistics/statistics/deleteD100a",
          deleteBatch: "/statistics/statistics/deleteBatchD100a",
        },
          loading: true,
          center: null,
          barData:[],
          loginfo:{},
          timeSelectOption:"7",
          trainingFields:['100米仰泳','100米自由泳','100米蝶泳','100米蛙泳','100米自测'],
          trainingInfo100:[],
        ...
        onChange (selectedValue) {
            ...
            if(params.mainId!=null) {
                getAction(this.url.chart, {mainId: params.mainId, selectValue: selectedValue}).then((res) => {
                    if (res.success) {
                        this.trainingInfo100 = res.result;
                    } else {
                        this.trainingInfo100 = null;
                    }
                })
            }
        },

      ...
      <a-row>
        <a-col :span="24">
          <a-card :loading="loading" activeTabKey="id" :bordered="false" title="训练成绩报表" :style="{ marginLeft:'10px',marginTop: '24px' }">
            ...
            <line-chart-multid :fields="trainingFields" :dataSource="trainingInfo400"></line-chart-multid>
          </a-card>
        </a-col>
      </a-row>
    </div>
    ...
    export default {
        ...
        data() {
            return {
                columns: [],
                url: {
                    list: "/statistics/statistics/listD400aByMainId",
                    chart:"/statistics/statistics/trainingInfo400",
                    delete: "/statistics/statistics/deleteD400a",
                    deleteBatch: "/statistics/statistics/deleteBatchD400a",
                },
                loading: true,
                center: null,
                barData:[],
                loginfo:{},
                timeSelectOption:"7",
                trainingFields:['400米仰泳','400米自由泳','400米蝶泳','400米蛙泳','400米自测'],
                trainingInfo400:[],
            ...
            onChange (selectedValue) {
                ...
                if(params.mainId!=null) {
                    getAction(this.url.chart, {mainId: params.mainId, selectValue: selectedValue}).then((res) => {
                        if (res.success) {
                            this.trainingInfo400 = res.result;
                        } else {
                            this.trainingInfo400 = null;
                        }
                    })
                }
            },

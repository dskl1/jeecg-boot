      ...
      <a-row>
        <a-col :span="24">
          <a-card :loading="loading" activeTabKey="id" :bordered="false" title="训练成绩报表" :style="{ marginLeft:'10px',marginTop: '24px' }">
            ...
            <line-chart-multid :fields="trainingFields" :dataSource="trainingInfo300"></line-chart-multid>
          </a-card>
        </a-col>
      </a-row>
    </div>
    ...
    export default {
        ...
        data() {
            return {
                columns: [],
                url: {
                    list: "/statistics/statistics/listD300aByMainId",
                    chart:"/statistics/statistics/trainingInfo300",
                    delete: "/statistics/statistics/deleteD300a",
                    deleteBatch: "/statistics/statistics/deleteBatchD300a",
                },
                loading: true,
                center: null,
                barData:[],
                loginfo:{},
                timeSelectOption:"7",
                trainingFields:['300米仰泳','300米自由泳','300米蝶泳','300米蛙泳','300米自测'],
                trainingInfo300:[],
            ...
            onChange (selectedValue) {
                ...
                if(params.mainId!=null) {
                    getAction(this.url.chart, {mainId: params.mainId, selectValue: selectedValue}).then((res) => {
                        if (res.success) {
                            this.trainingInfo300 = res.result;
                        } else {
                            this.trainingInfo300 = null;
                        }
                    })
                }
            },

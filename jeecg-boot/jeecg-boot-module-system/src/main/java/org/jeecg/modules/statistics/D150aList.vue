      ...
      <a-row>
        <a-col :span="24">
          <a-card :loading="loading" activeTabKey="id" :bordered="false" title="训练成绩报表" :style="{ marginLeft:'10px',marginTop: '24px' }">
            ...
            <line-chart-multid :fields="trainingFields" :dataSource="trainingInfo150"></line-chart-multid>
          </a-card>
        </a-col>
      </a-row>
    </div>
    ...
    export default {
        ...
        data() {
            return {
                columns: [],
                url: {
                    list: "/statistics/statistics/listD150aByMainId",
                    chart:"/statistics/statistics/trainingInfo150",
                    delete: "/statistics/statistics/deleteD150a",
                    deleteBatch: "/statistics/statistics/deleteBatchD150a",
                },
                loading: true,
                center: null,
                barData:[],
                loginfo:{},
                timeSelectOption:"7",
                trainingFields:['150米仰泳','150米自由泳','150米蝶泳','150米蛙泳','150米自测','150米自测'],
                trainingInfo150:[],
            ...
            onChange (selectedValue) {
                ...
                if(params.mainId!=null) {
                    getAction(this.url.chart, {mainId: params.mainId, selectValue: selectedValue}).then((res) => {
                        if (res.success) {
                            this.trainingInfo150 = res.result;
                        } else {
                            this.trainingInfo150 = null;
                        }
                    })
                }
            },

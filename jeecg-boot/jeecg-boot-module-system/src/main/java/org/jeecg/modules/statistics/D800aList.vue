      ...
      <a-row>
        <a-col :span="24">
          <a-card :loading="loading" activeTabKey="id" :bordered="false" title="训练成绩报表" :style="{ marginLeft:'10px',marginTop: '24px' }">
            ...
            <line-chart-multid :fields="trainingFields" :dataSource="trainingInfo800"></line-chart-multid>
          </a-card>
        </a-col>
      </a-row>
    </div>
    ...
    export default {
        ...
        data() {
            return {
                columns: [],
                url: {
                    list: "/statistics/statistics/listD800aByMainId",
                    chart:"/statistics/statistics/trainingInfo800",
                    delete: "/statistics/statistics/deleteD800a",
                    deleteBatch: "/statistics/statistics/deleteBatchD800a",
                },
                loading: true,
                center: null,
                barData:[],
                loginfo:{},
                timeSelectOption:"7",
                trainingFields:['800米仰泳','800米自由泳','800米蝶泳','800米蛙泳','800米自测'],
                trainingInfo800:[],
            ...
            onChange (selectedValue) {
                ...
                if(params.mainId!=null) {
                    getAction(this.url.chart, {mainId: params.mainId, selectValue: selectedValue}).then((res) => {
                        if (res.success) {
                            this.trainingInfo800 = res.result;
                        } else {
                            this.trainingInfo800 = null;
                        }
                    })
                }
            },
